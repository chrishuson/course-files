...
\fancyhead[LO]{BECA / Dr. Huson / Geometry 09-Congruence-transformations\\* pset ID: 163}

\begin{document}

\subsubsection*{9-9Exam-transformations}
\begin{enumerate}
...
\item The diagram below shows $\triangle ABC$. $E$ bisects $\overline{AB}$, and $\angle ACB \cong \angle AED$. $AB=20$, $AC=15$, and $DE=7$. Find the scale factor $k$, $BC$, and $AD$.
  \begin{multicols}{2}
    \begin{enumerate}
    \item $k=$ \vspace{1cm}
    \item $BC=$ \vspace{1cm}
  ...
  \begin{tikzpicture}[scale=1.0]
    \draw [thick]
    (0,0) node[above right] {$A$}--
    (230:6.66) node[below left] {$B$}--
    (260:5) node[below] {$C$}--cycle;
    \draw [thick]
    (230:3.33) node[above left] {$E$}--
    (260:4.44) node[right] {$D$};
  \end{tikzpicture}
\end{multicols}
