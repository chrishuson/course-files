...
\fancyhead[LO]{BECA / Dr. Huson / 10th Grade Geometry\\* Unit 6: Distance \& slope\\* 12 December 2019}
...
\newpage
  \item Given right $\triangle ABC$ with $AC=8.2, BC=5.1$, $m\angle C=90^\circ$. Find the value of $m\angle A$, expressed as a decimal to the nearest whole number.
  \begin{flushright}
    \begin{tikzpicture}[scale=0.7]
  ...
  \item Given right $\triangle JKL$ with $\overline{JK} \perp \overline{KL}$, $JK=17$, $m\angle J=28^\circ$. Find the length $KL$.
    \begin{flushright}
      \begin{tikzpicture}[scale=0.7]
        \draw [thick](-1,0)--(7,0)--(7,3)--cycle;
        \draw [fill] (-1,0) circle [radius=0.05] node[below]{$J$};
  ...
  \item Given right $\triangle ABC$ with $AC=8, BC=6$, $m\angle C=90^\circ$. Find the value of $m\angle B$, expressed as a decimal to the nearest thousandth.
  \begin{flushright}
    \begin{tikzpicture}[scale=0.7]
      \draw [thick](0,0)--(6,0)--(6,3)--(0,0);
      \draw [fill] (0,0) circle [radius=0.05] node[below]{$A$};
  ...
  \end{flushright}  \vspace{0.5cm}
    ...
    \item Spicy: Given a rectangle with area 48, width $x$, and length $x+8$.
      \begin{enumerate}
        \item Find $x$.\\[2cm]
