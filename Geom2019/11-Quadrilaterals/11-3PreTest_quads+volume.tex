   ...
   \item Three of the vertices of the parallelogram $JKLM$ are given: $J(2, 3)$, $K(8,2)$, $L(9, 6)$. Determine and state the coordinates of the fourth vertex, $M$, and mark and label it on the grid below. Draw the sides of the parallelogram.
     \begin{center}
       \begin{tikzpicture}[scale=.635]
         ...
         \fill (2,3) circle[radius=0.1cm] node[left]{$J$};
         \fill (8,2) circle[radius=0.1cm] node[right]{$K$};
         \fill (9,6) circle[radius=0.1cm] node[right]{$L$};
       \end{tikzpicture}
     \end{center}
...
\newpage
  \item A trapezoid has a longer base of 8 and shorter base of 7. One side is perpendicular to the base, and the other is at an angle, as shown. Its height is 2. \\[0.25cm]
  \begin{tikzpicture}[scale=1.7]
    ...
    \node at (6,1)[right]{$h=2$};
    \node at (3.5,2.1)[above]{$7$};
  \end{tikzpicture} \vspace{0.25cm}\\
  Determine and state the area of the trapezoid. \vspace{4cm}

  \item A circle with a diameter of 18 in and a central angle of $85^\circ$ is drawn below.
     \begin{enumerate}
       \item What is the area of the sector formed by the $85^\circ$ angle, to the \emph{nearest tenth of a square inch}?\\[0.25cm]
            \begin{tikzpicture}[scale=.5]
              \draw (0,0) circle[radius=5];
              \draw (40:5) node[above right]{$A$}--(220:5);
              %\draw (0,0)--(-40:5);
              \fill (0,0) circle[radius=0.1];
              \draw (0:2) node{$85^\circ$};
              \draw (120:0.75) node[rotate=45]{$18$ in};
              \draw [thick] (0,0)--(-40:5)node[right]{$B$} arc [start angle=-40, end angle=40, radius=5]
              --cycle;
            \end{tikzpicture}
        \vspace{1cm}
...
\newpage
  ...
  \begin{multicols}{2}
        \begin{tikzpicture}[scale=0.7]
          ...
          \node at (4,0)[below]{$5$};
          \node at (8,3)[right]{$4.9$};
          \node at (3.8,3)[above]{$7$};
        \end{tikzpicture}
        \begin{enumerate}
  ...
  \item A sailor observes the top of a lighthouse with an angle of elevation of $4^\circ$. She knows the lighthouse is 125 feet tall. Determine and state the distance $x$ between the sailor and the lighthouse, to the \emph{nearest foot}.\\[0.5cm]
    \begin{tikzpicture}[scale=1.1]
     ...
     \node at (10,1.2)[right]{height $=125$ feet};
     \node at (6,0)[below]{$x$};
...
\newpage
  ...
  \item Write down the center and radius of each circle, expressing the result as a simplified radical if necessary (not a decimal).
   \begin{enumerate}
     \begin{multicols}{2}
     \item   $(x-1)^2+(y-7)^2=100$ \vspace{2.5cm}
     \item   $x^2+(y+3)^2=20$ \vspace{2.5cm}
     \end{multicols}
   \end{enumerate} \vspace{2cm}

  \newpage
  \item In the diagram below, $\overline{AC}$ has endpoints with coordinates $A(-4,3)$ and $C(6,-2)$.
   \begin{center} %4 quadrant regents grid
     \begin{tikzpicture}[scale=.48]
       ...
       \draw [thick] (-4, 3)--(6,-2);
       \draw [fill] (-4, 3) circle [radius=0.1] node[above left] {$A$};
