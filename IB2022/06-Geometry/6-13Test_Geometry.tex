...
\item The function $T(x)=A \sin (Bx)+D$ is used to model weather for a Caribean island with $x$ the time in months.
  ...
  \begin{center}
    \begin{tikzpicture}[x=1cm, y=0.3cm, scale=0.75]
        ...
        \draw [thick, ->] (0,0) -- (0,32) node [right]{$T$};        \foreach \x in {0,1,...,15}
            \draw (\x cm,0) -- (\x cm,0) node[below] {$\x$};
        \foreach \y in {0,5,...,30}
            \draw[shift={(0,\y)}] (2pt,0pt)--(-2pt,0pt) node[left]{$\y$};
        \draw [thick, smooth,domain=0.:10] plot(\x,{5*(sin(deg(\x)))+20});
    \end{tikzpicture}
    \end{center}
