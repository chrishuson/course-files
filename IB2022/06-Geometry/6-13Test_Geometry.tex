...
\item The function $T(x)=A \sin (Bx)+D$ is used to model weather for a Caribean island with $x$ the time in months and $T(x)$ the daily high temperature in degrees Celsius.
  ...
  \begin{center}
    \begin{tikzpicture}[x=1cm, y=0.3cm, scale=0.75]
        %\draw [thin, color=lightgray, xstep=1.0cm,ystep=0.75cm] (0,1000) grid (10,1810);
        \draw [thick, ->] (0,0) -- (+12.4,0) node [above right]{$t$};
        \draw [thick, ->] (0,0) -- (0,32) node [right]{$T$};        \foreach \x in {0,1,...,12}
            \draw (\x cm,0) -- (\x cm,0) node[below] {$\x$};
        \foreach \y in {0,5,...,30}
            \draw[shift={(0,\y)}] (2pt,0pt)--(-2pt,0pt) node[left]{$\y$};
        \draw [thick, smooth,domain=0.:12] plot(\x,{5*(sin(deg(\x*pi/6)))+20});
        \node at (3,25){$\bullet$};
        \node at (3,25)[above]{$(3,25)$};
        \node at (9,15){$\bullet$};
        \node at (9,15)[below]{$(9,15)$};
    \end{tikzpicture}
    \end{center}
...
\begin{center}
  \begin{tikzpicture}[x=1cm, y=0.015cm, scale=0.55]
      \draw [thin, color=lightgray, xstep=1.0cm,ystep=0.75cm] (0,1000) grid (10,1810);
