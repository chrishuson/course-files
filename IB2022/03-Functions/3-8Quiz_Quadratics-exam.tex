...
\fancyhead[LO]{BECA / IB Math 03-Quadratic functions\\* 16 January 2022}

\begin{document}

\subsubsection*{3.8 Quiz: Linear and quadratic functions}
\begin{enumerate}
\item A linear function $f$ is graphed below.
    ...
    \end{enumerate} \vspace{.5cm}
      ...
      \begin{tikzpicture}[scale=0.8]
        ...
        \draw [thick, <->,smooth,samples=20,domain=-1.5:1.25] plot(\x,3*\x+2);
        \fill (1,5) circle[radius=0.1] node[below right]{$Q$};
      \end{tikzpicture}
      \end{center}
    \end{multicols}
    
\item Write the linear equation $\displaystyle y-2=\frac{1}{4}(x+8)$ in the form $y=mx+c$. \vspace{4cm}

\item Given $f(x)=(x-4)(x+2)$
...
\newpage
\item The function $f(x)=-x^{2}+4x$ is shown on the graph.
    \begin{enumerate}[itemsep=1.2cm]
      \begin{multicols}{2}
          ...
          \begin{tikzpicture}[scale=1]
            ...
            \foreach \y in {-4,...,-1,1,2,...,5} \draw (2pt,\y cm) -- (-2pt,\y cm) node[left] {$\y$};
            %\fill (-1,0) circle[radius=0.1] node[above left]{$j$};
            %\fill (3,0) circle[radius=0.1] node[above right]{$k$};
            \draw [thick, <->,smooth,samples=20,domain=-0.75:4.75] plot(\x,-\x*\x+4*\x);
          \end{tikzpicture}
          \end{center}
        \end{multicols}
