...
\item A function composed of four points $\{ (-2,1),(j,1),(3,4),(5,k) \}$ is plotted on the below.
    ...
    \item A ski jump is modeled by the cubic function $h(x)=7.25-2.2x+0.011x^{3}$ where $h$ is the height in meters above ground and $x$ is the horizontal distance (m).
    \begin{multicols}{2}
        \begin{tikzpicture}[x=1cm, y=0.5cm, scale=1]
            \draw [help lines] (-3,0) grid (3,10);
            \draw [thick, ->] (-3.2,0) -- (3.4,0) node [below right] {$x$};
            \draw [thick, ->] (0,-1.2)--(0,10.8) node [right] {$y$};
            \foreach \x in {-3,...,3}
                \draw[shift={(\x,0)}] (0,3pt)--(0,-3pt) node[below] {$\x$};
            \foreach \y in {1,2,...,10}
                \draw[shift={(0,\y)}] (2pt,0pt)--(-2pt,0pt) node[left]  {$\y$};
            \draw [-,thick,smooth,domain=-2.5:2.5] plot(\x,{-0.35*(\x)^3+0.8*(\x)^2+0.7*(\x)+1});
            \fill (-2.5,9.72) ellipse(3pt and 3pt) node [above right]{$(-2.5,9.72)$};
            \fill (15,11.375) ellipse(3pt and 3pt) node [above left]{$(15,11.375)$};
        \end{tikzpicture}
    \begin{enumerate}[itemsep=0.75cm]
        \item How wide is the ramp in meters?
