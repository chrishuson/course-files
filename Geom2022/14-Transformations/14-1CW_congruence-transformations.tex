...
\fancyhead[LO]{BECA / Mr. Segal, Dr. Huson / Geometry\\* Unit 14: Congruence transformations \\ 6 June 2022}

\renewcommand{\headrulewidth}{0pt}

\begin{document}
\subsubsection*{14.1 Classwork: Rigid motions, translation, reflection, rotation}
\begin{enumerate}
\item Slide $\triangle ABC$ to the left four and up five. Label the image $\triangle A'B'C'$.
  \begin{center}
      \begin{tikzpicture}[scale=.48]
      \draw [help lines] (-10,-5) grid (10,8);
      \draw [thick, <->] (-10.4,0) -- (10.4,0) node [right] {$x$};
      \draw [thick, <->] (0,-5.4)--(0,8.4) node [above] {$y$};  
      \draw [thick]
        (2,1) node[below left] {$A$}--
        (8,2) node[right] {$B$}--
        (6,-4) node[below right] {$C$}--cycle;  
    \end{tikzpicture}
  \end{center}

\item Apply the translation $(x,y) \rightarrow (x-1,y+7)$ to the point $P(-2,-4)$. \vspace{1cm}

\item Complete the t-table for the function $f$: $y=x^2$, plot the two missing points, labeling them as ordered pairs. 
\begin{center} 
\begin{tikzpicture}[scale=0.8]
  ...
  \foreach \y in {-2,-1,1,2,3,4,5} \draw (1pt,\y cm) -- (-1pt,\y cm) node[left] {$\y$};
  ...
  \end{tikzpicture}
  \end{center}
    The parabola $f$ is translated three to the right, $f \rightarrow g$. Complete the t-table for $g(x)=(x-3)^2$, plot the points, and draw a smooth curve.
...
\item What are the coordinates of the image of $B(2,5)$ after a reflection across the $x$-axis?  \vspace{0.25cm}
\begin{multicols}{2}
  \begin{enumerate}
  \item $(-2,5)$
  \item $(5,2)$
