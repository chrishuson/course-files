...
The following diagram shows a line representing the road and a circle representing the area covered by the tower's signal. Point R is on the circumference of the circle and points S and R are on the road. Point S is 38\,km from the tower and $\mathrm{R\hat{S}T} = 43^\circ$.
...
\begin{tikzpicture}[scale=0.06]
    ...
    \coordinate (S) at (-50,-25);

    % Draw road line
    \draw[thick] (-55,-30) -- (40,-30);

    % Draw ST
    \draw[thick, dashed] (0,0) -- (-50,-25);

    % Draw TR (radius)
    \draw[thick, dashed] (0,0) -- (20,-25);

    % Labels
    \node[below left] at (-50,-25) {S};
    \node[below] at (20,-30) {R};

    % Angle arc at S
    \draw (-45,-25) arc (0:25:5);
    \node at (-40,-20) {$43^\circ$};

    % Distance labels
    \node[above left] at (-25,-12) {38};
    \node[above right] at (10,-12) {32};

    % Note
    \node at (30,25) {\textit{diagram not to scale}};
\end{tikzpicture}
\end{center}

\begin{enumerate}[(a)]
    \item Let SR $= x$. Use the cosine rule to show that $x^2 - (76\cos 43^\circ)x + 420 = 0$.
...
% Shaded region using manual fill
\addplot[fill=gray!30, draw=none, domain=-2:2] {sqrt((4-x^2)/8)} \closedcycle;
% Curve outline
\addplot[domain=-2:2, blue, thick] {sqrt((4-x^2)/8)};
\end{axis}
\node at (5.5,3.5) {\textit{diagram not to scale}};
\end{tikzpicture}
\end{center}

A container can be modelled by rotating this region by $360^\circ$ about the $x$-axis.
