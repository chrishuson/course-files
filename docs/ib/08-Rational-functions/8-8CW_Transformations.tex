...
\textbf{direction}, and the \textbf{magnitude}.
...
\begin{enumerate}[itemsep=0.30cm, label=(\alph*)]
  ...
  \item A vertical \underline{\hspace{2.8cm}} with \underline{\hspace{2.8cm}} of $2$ doubles every $y$-coordinate.
...
in words}, the sequence of transformations in order, and write the equations of the two
asymptotes.
...
as \textbf{dashed} lines and label them with their equation, then plot at least two points.
...
\begin{center}
\begin{tikzpicture}[scale=0.6]
  \draw [thin, color=gray, step=1.0cm] (-6.4,-6.4) grid (5.4,6.4);
  \foreach \x in {-6,-5,-4,-3,-2,-1,1,2,3,4,5} \draw[shift={(\x,0)}] (0pt,-2pt)--(0pt,2pt) node[below] {$\x$};
  \foreach \y in {-6,-5,-4,-3,-2,-1,1,2,3,4,5,6} \draw[shift={(0,\y)}] (2pt,0pt)--(-2pt,0pt) node[left] {$\y$};
  \draw [thick, ->] (-6.6,0) -- (5.8,0) node [right] {$x$};
  \draw [thick, ->] (0,-6.6) -- (0,6.6) node [right] {$y$};
\end{tikzpicture}
\end{center}
...
\medskip
\noindent Read $f\circ g$ as ``f composed with g'' ($g$ goes first, then $f$ wraps around it):
...
\noindent\textbf{E3.\quad Find the outer function.}\quad Given $g(x)=x-3$ and
