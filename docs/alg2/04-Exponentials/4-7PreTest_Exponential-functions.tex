...
\begin{multicols}{2}
    ...
    \end{enumerate}
    \begin{center}
    \begin{tikzpicture}[x=1cm, y=0.0005cm, xscale=1]
        \draw [thin, color=lightgray, xstep=1cm,ystep=1cm] (0,0) grid (5,14050);
        \draw [thick, ->] (0,0) -- (+5.3,0) node [below]{$t$};
        \draw [thick, ->] (0,0) -- (0,15000) node [right]{$y$};        
        \foreach \x in {0,1,...,5}
            \draw (\x cm,0) -- (\x cm,0) node[below] {$\x$};
        \foreach \y in {0,2000,...,14000}
            \draw[shift={(0,\y)}] (2pt,0pt)--(-2pt,0pt) node[left]{\num[group-separator={,}]{\y}};
