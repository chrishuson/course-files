...
\item Solve for $x$ algebraically: \(\displaystyle \frac{x}{x-6} + \frac{x}{x-2} = \frac{4}{x^2-8x+12}\) %August 2023 Regents
...
\item Graph $y = x^3 - 4x^2 + 2x + 7$ on the set of axes below.
    ...
    \begin{tikzpicture}[scale=0.7]
        ...
        % Use the appropriate function here
        %\draw[domain=-10:10,samples=100] plot (\x,{(\x)^3 - 4*(\x)^2 + 2*(\x) + 7});
    \end{tikzpicture}
    \end{center}
