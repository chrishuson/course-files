...
\lhead{BECA / Dr. Huson / 10.3 Geometry\\* 27 November 2018}

\renewcommand{\headrulewidth}{0pt}

\begin{document}
\subsection*{Algebra 1 Regents Standards: learning trajectories}

  \begin{enumerate}

\subsubsection*{Rational \& irrational numbers, closure}

  \item Calculate the given sum with a calculator, identifying and explaining rational numbers (``terminating or repeating") versus irrationals (``non-terminating, non-repeating")

  \item State whether each value is ``rational because it can be expressed as a fraction" or ``irrational because it can NOT be expressed as a fraction"
    \begin{enumerate}
      \item 7.1 \vspace{2cm}
      \item $\sqrt{12}$ \vspace{2cm}
      \item 5.25 \vspace{2cm}
      \item $\sqrt{25}$ \vspace{2cm}
    \end{enumerate}

  \item A teacher wrote the following set of numbers on the board:
    \begin{center}
      $a=\sqrt{20}$ \hspace{1cm} $b=2.5$ \hspace{1cm} $c=\sqrt{225}$
    \end{center}
    Explain why $a+b$ is irrational, but $b+c$ is rational. \vspace{2cm}

    \item Is the product of $\sqrt{16}$ and $\displaystyle \frac{4}{7}$ rational or irrational? Explain your reasoning.
