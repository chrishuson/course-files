...
\item Do Not Solve. Circle the appropriate equation, cite a  justification:
  ...
  \item $\overleftrightarrow{RPU}$ with ray $\overrightarrow{PS}$. \hspace{6cm}
    ...
    \end{tikzpicture}\\[0.5cm]
    $\angle UST \cong \angle RSU$ \hspace{0.5cm} m$\angle UST + \text{m}\angle RSU =  180$ \hspace{0.5cm} \rule{6cm}{0.15mm} \vspace{0.25cm}
  ...
  \item Given $\overrightarrow{BA} \perp \overrightarrow{BC}$, $m \angle ABD = 2x-5$, and $m \angle DBC = x-10$.
    ...
    \end{tikzpicture}\\[0.5cm]
    $\angle ABD \cong \angle DBC$ \hspace{0.5cm} m$\angle ABD + \text{m}\angle DBC =  90$ \hspace{0.5cm} \rule{6cm}{0.15mm}
  \end{enumerate}
