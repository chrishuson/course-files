...
\begin{frame}{Angle terminology and notation}
  ...
  \end{columns} \bigskip
  \begin{description}
    \item[Right angle] Angle measuring $90^\circ$, mark as small square
...
\section{2.2 Angle addition \hfill 29 September}
  \begin{frame}{Learning Target: I can solve for angle measures}
    {CCSS: HSG.CO.A.1 Know precise geometric definitions \hfill \alert{2.2 Thursday 29 Sept}}
      Do Now: Given $\overline{LMN}$, $LM=2x-3$, $MN=x+7$, $LN=4x$. Find $x$
        \begin{flushleft}
          \begin{tikzpicture}
          \draw[thick] (0,0)--(7,0);
          ...
          \draw[<->, dashed] (0,-1)--(7,-1);
          \node at (3.5,-1) [below]{$4x$};
          \end{tikzpicture}
        \end{flushleft}
        \vspace{2cm}
      \emph{Don't forget to check the solution.} \par \bigskip 
      What is the name of the geometry postulate this problem requires?
    \end{frame}

\begin{frame}{Angle addition postulate}
  $m\angle ABD=30^\circ$, $m\angle DBC=45^\circ$. Find $m\angle ABC$. \vspace{0.5cm}
    \begin{center}
      \begin{tikzpicture}
