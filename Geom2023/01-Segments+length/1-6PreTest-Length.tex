...
\begin{enumerate}
  ...
  $AC=$ \rule{2cm}{0.15mm} $+$ \rule{2cm}{0.15mm} $=$ \rule{2cm}{0.15mm}
...
\item As diagrammed below, point $M$ is the midpoint of $\overline{AB}$, $AM=4x$, $MB=x+15$. Solve for $x$. (show the check for full credit)
  \begin{center}
    \begin{tikzpicture}
      \draw[thick] (0,0)--(6,0);
      \draw[fill] (0,0) circle [radius=0.05] node[below]{$A$};
