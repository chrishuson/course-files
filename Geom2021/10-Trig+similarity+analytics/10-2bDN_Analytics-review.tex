...
\fancyhead[LO]{BECA / Dr. Huson / Geometry 10-Trig+similarity+analytics\\* pset ID: 168}
...
\item Write down the slope parallel or perpendicular to the given slope. \vspace{0.5cm}
  \begin{enumerate}
    \begin{multicols}{2}
      \item   $m= 0.75 \hspace{1cm} m_{\parallel} = $ \vspace{1cm}
      \item   $m= -\frac{1}{2} \hspace{1cm} m_{\parallel} = $
...
\newpage
\item What is an equation of the perpendicular bisector of $\overline{AB}$ with $A(-2,5)$ and $B(4,-1)$? 
  \begin{flushright}
...
\newpage
\item Find the coordinates of the image of $G(4,1)$ after a rotation of $90^\circ$ around the origin.
  \begin{flushright}
    \begin{tikzpicture}[scale=.48]
    \draw [help lines] (-5,-5) grid (5,5);
    \draw [thick, <->] (-5.4,0) -- (5.4,0) node [right] {$x$};
...
\item Find the coordinates of the image of $G(2,5)$ after a reflection across the $y$-axis.
  \begin{flushright}
    \begin{tikzpicture}[scale=.48]
    \draw [help lines] (-5,-5) grid (5,5);
    \draw [thick, <->] (-5.4,0) -- (5.4,0) node [right] {$x$};
